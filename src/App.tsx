...
export default function App() {
  ...
  return (
    <div className="flex h-screen overflow-hidden text-[#0F172A] font-sans bg-[#F8FAFC]">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-64 border-r bg-white flex-col p-6 space-y-8 shrink-0">
        ...
        <div className="pt-6 border-t space-y-3">
          <button onClick={handleExport} className="w-full text-sm font-bold text-slate-400 hover:text-slate-600 flex items-center justify-center space-x-2 transition-colors">
            <Download className="w-4 h-4" /> <span>Dışa Aktar</span>
          </button>
          <label className="w-full text-sm font-bold text-slate-400 hover:text-slate-600 flex items-center justify-center space-x-2 transition-colors cursor-pointer">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="p-6 md:p-8 flex justify-between items-center bg-white/50 backdrop-blur-sm sticky top-0 z-20">
          ...
          <div className="hidden md:flex items-center space-x-4">
            <div className="text-right text-sm">
              ...
              <div className="text-xl font-black text-slate-900">{(stats.tytAvg + stats.aytAvg).toFixed(2)}</div>
            </div>
            <div className="w-12 h-12 rounded-full bg-slate-900 flex items-center justify-center text-white font-black">
              {stats.total}
            </div>
          </div>
        </header>
              ...
              {view === 'dashboard' && (
                ...
                >
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    <div className="lg:col-span-4 bg-brand rounded-3xl p-6 text-white shadow-xl shadow-indigo-200">
                    ...
                    <div className="lg:col-span-4 bg-white rounded-3xl p-6 border shadow-sm">
                      <div className="text-slate-400 text-xs font-black uppercase tracking-widest">TYT Ortalaması</div>
                      <div className="text-6xl font-black mt-2 text-slate-900">{stats.tytAvg.toFixed(1)}</div>
                    ...
                    <div className="lg:col-span-4 bg-white rounded-3xl p-6 border shadow-sm">
                      ...
                      <div className="text-6xl font-black mt-2 text-slate-900">{stats.aytAvg.toFixed(1)}</div>
                  ...
                  <section>
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-2xl font-black tracking-tight">SON DENEMELER</h3>
                      <button 
                        onClick={() => setView('history')}
                    ...
                    <div className="space-y-4">
                      {stats.recent.length > 0 ? (
                        stats.recent.map(exam => (
                          <ExamRow key={exam.id} exam={exam} onDelete={deleteExam} />
                        ))
                      ) : (
                        <div className="text-center py-20 bg-white rounded-[2rem] border border-dashed border-slate-200">
                          ...
                          <button 
                            onClick={() => setView('add')}
                            className="mt-6 px-8 py-4 bg-brand text-white rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-indigo-100"
                          >
                            Hemen Başla
                          </button>
                        </div>
              ...
              {view === 'analysis' && (
                ...
                >
                  <div className="flex items-center justify-between">
                    <h2 className="text-3xl font-black tracking-tight uppercase italic">Eksiklerim</h2>
                    <div className="flex bg-white p-1 rounded-2xl border shadow-sm">
                      {['TYT', 'AYT'].map(t => (
                        <button 
                          key={t}
                          onClick={() => setAnalysisType(t as any)}
                          className={`px-8 py-2 text-xs font-black rounded-xl transition-all uppercase tracking-widest ${analysisType === t ? 'bg-slate-900 text-white' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                          {t}
                        </button>
                      ))}
                    ...
                    {analysisData.length > 0 ? (
                      <div className="space-y-4">
                        {analysisData.map((item, idx) => (
                          <div key={item.subject} className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl hover:bg-slate-50 transition-colors group">
              ...
              {view === 'history' && (
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <h2 className="text-3xl font-black tracking-tight uppercase">Tüm Liste</h2>
                    <div className="flex items-center gap-2 bg-white p-1 rounded-2xl border shadow-sm self-start">
                      {['ALL', 'TYT', 'AYT'].map(t => (
                        <button 
                          key={t}
                          onClick={() => setFilterType(t as any)}
                          className={`px-6 py-2 text-xs font-black rounded-xl transition-all uppercase tracking-widest ${filterType === t ? 'bg-slate-900 text-white' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                          {t === 'ALL' ? 'Hepsi' : t}
                        </button>
                      ))}
        ...
        {/* Mobile Bottom Nav */}
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-200 px-6 py-3 z-50">
...
// Sidebar Button
function SidebarBtn({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactElement, label: string }) {
  return (
    <button 
      onClick={onClick}
      className={`w-full flex items-center space-x-3 px-4 py-3 rounded-2xl font-bold transition-all transition-all duration-300 ${active ? 'bg-brand-light text-brand shadow-sm shadow-indigo-100' : 'text-slate-500 hover:bg-slate-50'}`}
    >
...
function ExamRow({ exam, onDelete, onEdit, showDetails = false }: { exam: Exam, onDelete: (id: string) => void, onEdit: (id: string) => void, showDetails?: boolean }) {
  ...
  return (
    <div className="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm transition-all hover:shadow-md">
      <div className="p-5 flex items-center justify-between cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xs ${exam.type === 'TYT' ? 'bg-indigo-50 text-brand' : 'bg-orange-50 text-orange-600'}`}>
            {exam.type}
          </div>
          <div>
        ...
        <div className="flex items-center gap-2 md:gap-6">
          <div className="text-right hidden sm:block">
            <p className="text-[10px] font-black uppercase text-slate-300 tracking-[0.2em]">Net Skoru</p>
            <p className="text-2xl font-black text-slate-900 leading-none">{exam.net.toFixed(2)}</p>
          </div>
          <div className="flex items-center gap-1">
            <button 
              onClick={(e) => { e.stopPropagation(); onEdit(exam.id); }}
              className="p-3 hover:bg-indigo-50 text-brand rounded-2xl transition-colors"
            >
              <Plus className="w-4 h-4 rotate-45" style={{ transform: 'rotate(0)' }} />
              {/* Using a simple edit icon approximation since I can't easily find Edit in the predefined set above without re-importing */}
          ...
          {isExpanded ? <ChevronDown className="w-5 h-5 opacity-40 shrink-0" /> : <ChevronRight className="w-5 h-5 opacity-40 shrink-0" />}
        </div>
      </div>
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div 
            ...
            className="border-t border-slate-50 bg-slate-50/30 p-6"
          >
            <div className="grid grid-cols-3 gap-6 mb-6">
              <div className="bg-white p-4 rounded-[1.5rem] border border-slate-100 flex flex-col items-center">
                <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Doğru</span>
                <span className="text-2xl font-black">{exam.totalCorrect}</span>
              </div>
              <div className="bg-white p-4 rounded-[1.5rem] border border-slate-100 flex flex-col items-center">
                <span className="text-[10px] font-black text-red-500 uppercase tracking-widest">Yanlış</span>
                <span className="text-2xl font-black">{exam.totalWrong}</span>
              </div>
              <div className="bg-white p-4 rounded-[1.5rem] border border-slate-100 flex flex-col items-center">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Boş</span>
                <span className="text-2xl font-black">{exam.totalEmpty}</span>
              </div>
            </div>
              ...
              <div className="mt-6 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {exam.questions.map((q, idx) => {
                    ...
                    return (
                      <div key={idx} className="bg-white p-3 rounded-2xl border border-slate-100 text-xs flex items-center justify-between group">
                        <div className="flex items-center gap-3">
                          <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black ${q.status === 'wrong' ? 'bg-red-50 text-red-500' : 'bg-slate-100 text-slate-500'}`}>
                            {idx + 1}
                          </span>
...
function ExamForm({ onSave, onCancel, initialData }: { onSave: (exam: Exam) => void, onCancel: () => void, initialData?: Exam }) {
  ...
  // Helper to render questions grouped by section
  const renderQuestionsBySection = () => {
    ...
    return sections.map((section, sIdx) => {
      ...
      for (let i = 0; i < section.count; i++) {
        const currentGlobalIdx = sectionStart + i;
        sectionElements.push(
          <QuestionItem 
            key={currentGlobalIdx}
            index={i} // Relative index (0-based, will add 1 in component)
      ...
      return (
        <div key={sIdx} className="space-y-4 mb-6">
          <div className="py-3 border-b-2 border-slate-100">
            <h3 className="text-sm font-black text-brand uppercase tracking-[0.2em]">{section.name} <span className="text-slate-400 font-bold">({section.count} Soru)</span></h3>
          </div>
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="text-center space-y-2">
          <h2 className="text-4xl font-black tracking-tight uppercase">DENEME TÜRÜ SEÇİN</h2>
          <p className="text-slate-400 font-medium">Hangi oturum için deneme girişi yapmak istiyorsunuz?</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <button 
            onClick={() => setType('TYT')}
            className="group relative bg-white border-2 border-slate-100 p-10 rounded-[2.5rem] shadow-sm hover:border-brand hover:shadow-xl hover:shadow-indigo-100 transition-all text-left overflow-hidden"
          >
            ...
            <div className="relative z-10 space-y-4">
              ...
              <h3 className="text-2xl font-black uppercase tracking-tight">Temel Yeterlilik</h3>
          ...
          <button 
            onClick={() => setType('AYT')}
            className="group relative bg-white border-2 border-slate-100 p-10 rounded-[2.5rem] shadow-sm hover:border-orange-500 hover:shadow-xl hover:shadow-orange-100 transition-all text-left overflow-hidden"
          >
            ...
            <div className="relative z-10 space-y-4">
              ...
              <h3 className="text-2xl font-black uppercase tracking-tight">Alan Yeterlilik</h3>
              <p className="text-slate-500 text-sm font-medium leading-relaxed">
  ...
  return (
    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm max-w-4xl mx-auto mb-10 overflow-hidden">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div className="flex items-center gap-4">
          ...
          >
            <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 group-hover:text-black transition-colors">
              <Plus className="w-5 h-5 rotate-45" />
            </div>
          </button>
          <h2 className="text-3xl font-black tracking-tight uppercase italic">{type} Denemesi</h2>
        </div>
        <div className="hidden md:flex bg-slate-100 p-1 rounded-2xl">
          <div className={`px-8 py-3 text-sm font-black rounded-xl uppercase tracking-widest transition-all ${type === 'TYT' ? 'bg-white shadow-md text-brand' : 'text-slate-400 opacity-50'}`}>
            TYT MODU
          </div>
          <div className={`px-8 py-3 text-sm font-black rounded-xl uppercase tracking-widest transition-all ${type === 'AYT' ? 'bg-white shadow-md text-orange-600' : 'text-slate-400 opacity-50'}`}>
            AYT MODU
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input 
              ...
              className="w-full bg-slate-50 border-2 border-transparent focus:border-brand/20 focus:bg-white transition-all rounded-2xl px-6 py-4 text-sm font-bold focus:outline-none"
            />
          </div>
          <div className="space-y-2">
        ...
        <div className="bg-slate-50/50 rounded-[2rem] p-6 max-h-[600px] overflow-y-auto border border-slate-100 relative">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 pt-6">
          <button 
            type="submit"
            className="w-full sm:flex-1 bg-brand text-white font-black py-5 rounded-[2rem] shadow-xl shadow-indigo-100 hover:scale-[1.02] transition-all active:scale-95 uppercase tracking-widest"
          >
            Denemeyi Kaydet
          </button>
          <button 
            type="button" 
            onClick={onCancel}
            className="w-full sm:w-auto px-10 bg-slate-100 text-slate-500 font-black py-5 rounded-[2rem] hover:bg-slate-200 transition-all uppercase tracking-widest text-xs"
          >
            Vazgeç
          </button>
        </div>
...
function QuestionItem({ index, globalIndex, entry, subjects, onChange }: { index: number, globalIndex: number, entry: QuestionEntry, subjects: string[], onChange: (u: Partial<QuestionEntry>) => void }) {
  const [showNote, setShowNote] = useState(false);

  if (!entry) return null;

  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-3 shadow-sm hover:border-brand/20 transition-all">
      <div className="flex flex-col md:flex-row md:items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <select 
            ...
            className={`w-full bg-slate-50 text-[11px] font-black uppercase tracking-widest border-2 transition-all rounded-xl py-2 px-3 cursor-pointer outline-none ${!entry.subject ? 'border-amber-200 text-amber-600' : 'border-transparent text-slate-700'}`}
          >
        ...
        <div className="shrink-0 flex justify-end">
          <button 
            ...
            className={`p-2 rounded-xl transition-all ${entry.note ? 'text-brand bg-brand-light shadow-sm' : 'text-slate-300 hover:bg-slate-50'}`}
          >
        ...
        {showNote && (
          ...
          >
            <textarea 
              ...
              className="w-full bg-slate-50 border-2 border-transparent focus:border-brand/10 rounded-xl p-3 text-xs font-bold focus:outline-none placeholder:text-slate-300"
            />
          </motion.div>
        )}
      </AnimatePresence>
